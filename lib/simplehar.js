var harToHtml = function(args) {
	...
	if(args.return === true) {
		...
		html = harToHtml.mergingRequests(requestsHtml, html, har);
...
harToHtml.requestsToHtml = function(harParsed, htmlTemplate) {
	...
	var html =  '',
		i = 0,
		ilen = harParsed, request, prop, template;
	
	
	for(i = 0, ilen = har.length; i < ilen; i++) {
		request = harParsed[i];
...
harToHtml.mergingRequests = function(htmlTemplate, requests, har) {
	'use strict';
	var tpl, css, js,
		replacer = "$1",
		translations = path.join(path.dirname(__filename), '..', 'src', 'translate.json'),
		lng = args.lng || false,
		html = requests
				.toString()
