module.exports = function(args) {
	...
	var fs = require('fs'),
		...
		simpleharSrc = path.join(path.dirname(args.html), 'simpleharSrc'),
		...
		readHar = function(har) {
			...
			var encode = function() {
				var Ent, ent;
				try {
					Ent = require('html-entities').XmlEntities;
					ent = new Ent();
					return ent.encode;
				}
				catch(e) {
			...
			return hth(har, encode());
		},
		fillTemplates = function(template, har) {
			var html =  '',
				i = 0,
				ilen = har.length,
				prop, nHar, _html;
			...
			for(;i<ilen;i++) {
				...
				if(nHar.tabs && nHar.tabContainers && nHar.fileContent !== '' && (nHar.mime == 'css' || nHar.mime.indexOf('javascript') != -1)) {
					nHar.tabs += '<li><a href="#parsedcontent">[Parsed Content]</a></li>';
					nHar.tabContainers += '<div class="parsedcontent hidden" style=""><pre class="pre-scrollable">';
					
					if(nHar.mime == 'css')
						nHar.tabContainers += unminify.css(nHar.fileContent);
					else if(nHar.mime.indexOf('javascript') != -1)
						nHar.tabContainers += unminify.js(nHar.fileContent);
					
					nHar.tabContainers += '</pre></div>';
				}
		...
		generateHtml = function(container, html, har) {
			html = container
					.toString()
					.replace('{har}', html)
	...
	var harParsed, htmlFilled, htmlFinal;
	
	if(args['return'] === true) {
		harParsed = readHar(fs.readFileSync(args.har));
		htmlFilled = fillTemplates(fs.readFileSync(path.join(srcFolder, 'requestTemplate.html')), harParsed);
		htmlFinal = generateHtml(fs.readFileSync(path.join(srcFolder, 'template.html')), htmlFilled, harParsed);
		...
		fs.readFile(args.har, function(err, har) {
			...
			var harParsed = readHar(har);
			
			
			
			fs.readFile(path.join(srcFolder, 'requestTemplate.html'), function(err, requestTemplate) {
				if(err) throw err;
				
				var htmlFilled = fillTemplates(requestTemplate, harParsed);
				
				
				
				fs.readFile(path.join(srcFolder, 'template.html'), function(err, template) {
					if(err) throw err;
					
					var htmlFinal = generateHtml(template, htmlFilled, harParsed);
					
					
					fs.writeFileSync(args.html, htmlFinal);
