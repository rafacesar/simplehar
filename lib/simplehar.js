var harToHtml = function(args) {
	'use strict';
	var fs = require('fs'),
		path = require('path'),
		har = args.har,
		baseFolder = path.join(path.dirname(__filename), '..'),
		srcFolder = path.join(baseFolder, 'src'),
		harParser = require(path.join(srcFolder, 'harParser.js')),
		html = 'template.html',
		requestsHtml = 'requestTemplate.html';
	
	
	if(args.return === true) {
		har = fs.readFileSync(har);
		
		har = harToHtml.readHar(har);
		
		har = harParser(har.har, har.encoder);
		
		
		requestsHtml = path.join(srcFolder, requestsHtml);
		requestsHtml = fs.readFileSync(requestsHtml);
		
		requestsHtml = harToHtml.requestsToHtml(har, requestsHtml.toString());
		
		
		html = path.join(srcFolder, html);
		html = fs.readFileSync(html);
		
		html = harToHtml.mergingRequests(requestsHtml, html, har, args);
	...
	fs.readFile(args.har, function(err, har) {
		...
		har = harToHtml.parseHar(har);
		
		fs.readFile(path.join(srcFolder, requestsHtml), function(err, requestsHtml) {
			if(err) throw err;
			
			requestsHtml = harToHtml.requestsToHtml(har, requestsHtml.toString());
			
			fs.readFile(path.join(srcFolder, html), function(err, html) {
				if(err) throw err;
				
				html = harToHtml.mergingRequests(requestsHtml, html, har, args);
				
				if(typeof args.return == 'function')
					fs.writeFile(args.html, html, args.return);
				else
...
harToHtml.readHar = function(har) {
	'use strict';
...
harToHtml.requestsToHtml = function(harParsed, htmlTemplate) {
	'use strict';
	var html =  '',
		i = 0,
		ilen = harParsed, request, prop, template, mime;
	
	
	for(i = 0, ilen = harParsed.length; i < ilen; i++) {
		request = harParsed[i];
		
		mime = request.mime;
		
		if(mime && (mime.toLowerCase() === 'css' || mime.toLowerCase().indexOf('javascript') != -1))
			request = harToHtml.beautify(request);
		
		template = htmlTemplate;
		
		for(prop in request)
			template = template.replace(new RegExp('{' + prop + '}', 'g'), request[prop]);
...
harToHtml.beautify = function(request) {
	'use strict';
	var mime = request.mime.toLowerCase();
	
	if(mime.indexOf('javascript') != -1)
		mime = 'js';
	
	if(request.tabs
	&& request.tabContainers
	&& request.fileContent) {
		
		request.tabs += '<li><a href="#parsedcontent">[Parsed Content]</a></li>';
		request.tabContainers += '<div class="parsedcontent hidden"><pre class="pre-scrollable">';
		
		request.tabContainers += unminify[mime](request.fileContent);
		
		request.tabContainers += '</pre></div>';
	}
	
	return request;
};
harToHtml.mergingRequests = function(htmlTemplate, requests, har, _args) {
	'use strict';
	var tpl, css, js,
		replacer = "$1",
		path = require('path'),
		fs = require('fs'),
		simpleharSrc,
		srcFolder = path.join(path.dirname(__filename), '..', 'src'),
		translations = path.join(srcFolder, 'translate.json'),
		lng = _args.lng || false,
		html = requests
				.toString()
				.replace('{har}', htmlTemplate)
				.replace('{info}', har.info)
				.replace('{title}', har.title),
	...
	if(fs.existsSync(translations) && lng) {
		translations = fs.readFileSync(translations);
	...
	if(!_args.frame) {
		...
		tpl = fs.readFileSync(path.join(path.dirname(__filename), '..', 'index.html'));
		simpleharSrc = path.join(path.dirname(_args.html), 'simpleharSrc')
		html = tpl
			.toString()
		...
		if(_args.return === true && _args.frameContent) {
			if(_args.frameContent.css !== false)
				html = html.replace('{style}', '<style>' + css + '</style>');
			
			if(_args.frameContent.js !== false)
