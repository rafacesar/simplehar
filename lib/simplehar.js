var fs = require('fs'),
	...
	simplehar = function(args) {
		...
		var har = args.har,
			...
			html, filename, hars, i, ilen, execAsync;
		...
		if(args.callback) {
			...
			simplehar.readHar(har, function(har) {
				...
				for(i=0,ilen=hars.length;i<ilen;i++) {
					...
					simplehar.requestsToHtml(har, execAsync);
...
simplehar.htmlFilename = function(har, html, multi, commandLine) {
	...
	var htmlFilename;
	if(multi) {
		if(commandLine || html.indexOf('{id}') !== -1) {
			htmlFilename = url.parse(har.title);
			htmlFilename = htmlFilename.hostname + '-' + har.pRef;
			htmlFilename = path.join(path.dirname(html), htmlFilename + '.html');
		}
		else {
			htmlFilename = simplehar.simpleTemplater({
				id:har.pRef,
				url:url.parse(har.title).hostname
			}, html);
		}
	}
	else
