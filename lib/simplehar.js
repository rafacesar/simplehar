var simplehar = function(args) {
	...
	//Returning content instead of creating files
	if(args.return === true) {
		...
		requestsHtml = simplehar.requestsToHtml(har, requestsHtml.toString());
		
		
		html = path.join(srcFolder, html);
		html = fs.readFileSync(html);
		
		html = simplehar.mergingRequests(requestsHtml, html, har, args);
...
simplehar.readHar = function(har) {
	'use strict';
...
simplehar.requestsToHtml = function(harParsed, htmlTemplate) {
	'use strict';
	var html =  '',
		i = 0,
	...
	for(i = 0, ilen = harParsed.length; i < ilen; i++) {
		...
		if(mime && (mime.toLowerCase() === 'css' || mime.toLowerCase().indexOf('javascript') != -1))
			request = simplehar.beautify(request);
...
simplehar.beautify = function(request) {
	'use strict';
...
simplehar.mergingRequests = function(htmlTemplate, requests, har, _args) {
	'use strict';
	var tpl, css, js,
		replacer = "$1",
		path = require('path'),
